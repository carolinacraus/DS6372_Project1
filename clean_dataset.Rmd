...
car_df <- car_df %>% filter(MSRP <=80000)

car_df$log_MSRP <- log(car_df$MSRP)
car_df$Year<- as.integer(car_df$Year)
...
Simple Model - with Training set of data
```{r, message=FALSE, warning=FALSE, fig.align='center'}
library(car)
...
Simple Model - with complete dataset
```{r, message=FALSE, warning=FALSE, fig.align='center'}
library(car)
...
- compare KNN agaisnt complex model created. 
- gather measures of fit for all models (MSE, R^2, Adj.R, AIC, BIC)




<h1> ask for clarification </h1>
feat. selection and 5 fold cross validation on entire dataset
```{r}
library(caret)
library(ggcorrplot)
...
opt.pen<- glmnet.fit2$finalModel$lambdaOpt
...
Complex Model - with training dataset
```{r}

complex_fit <- lm(log_MSRP~log(Year)+log(Engine.HP),training)
...
implement KNN regression (or other model)
```{r}
predictors <- training()
...
plot3d(Adver$TV,Adver$radio,Adver$sales,xlab="TV 5",ylab="Radio 5",zlab="Sales 5")
surface3d(0:300,0:50,pred.surface,alpha=.4)


predictors # dataframe
knn_training <- training[,-c(1,3,5,6,7,9:11)] 
knn_validation <- validate[,-c(1,3,5,6,7,9:11)]
dim(knn_training)
knn5 <- knn.reg(knn_training, test=knn_validation, y = knn_training$log_MSRP, k = 5)
pred.surface<- matrix(knn5$pred,7496,8)
plot3d(knn_training$log_MSRP)
```
