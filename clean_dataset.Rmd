...
engineering features - vehicle.style
```{r}
...
engineering features - market.category
```{r}
...
car_model_fit <- lm(log(MSRP)~Year+log(Engine.HP)+Popularity,training)
...
glm_df <- training[,!names(training) %in% c("Engine.Cylinders4","Engine.Cylinders12","Engine.Cylinders16",
                                            "Driven_Wheelsrear wheel drive", "flex_Fuel_intake",
                                            "nat_fuel_intake","electric_intake","french_build",
                                            "Netherlands_build", "Engine.HP","log_MSRP")]
...
glm_model_2 <- lm(log(MSRP)~., glm_without_outliers)
...
training2 <- training_b[,-c(1,3,5:7,9:11)]
validate2 <- validate_b[,-c(1,3,5:7,9:11)]

knn_reg <- knn.reg(train = training2, test = validate2, y=training2$MSRP, k=5)
...
val_predictions <- predict(knn.fit, newdata = validate2)
# Root MSE 
# MAE
# plot the coefficents in glmnet
```
