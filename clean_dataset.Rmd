...
library(tree)
library(rpart)
dataset <- read.csv("C:/Users/caroc/OneDrive/Desktop/DS6372_Project1/data1.csv")
...
No Feature Selection Algorithms were used to create this model as the objective was to create model with high simplicity, and intuitive relationships. 
...
- Holding Engine.HP constant, a one year increase is associated with a multiplicative change of e^.01245 = 1.013 or 1.3% increase in the median MSRP. We are 95% confident that the multiplicative change in the median MSRP is contained in the interval (e^.01189, e^.01399) = (1.012, 1.014), or an increase of (1.2%, 1.4%). 

- Holding The Year Constant, A doubling of Engine.HP is associated with a 2^.8458 = 1.797 multiplicative change in the median of MSRP, or a 80% increase. We are 95% confident that the multiplicative change in the median MSRP is contained in the interval (2^.8318, 2^.8598) = (1.78, 1.81), or an increase between (78%, 81%). 


*Practical Significance | Year Predictor:*
- A one year increase (a 1 year more current vehicle) car will result in a estimated 1.3% increase in the median MSRP with a 95% confidence interval of 1.2%-1.4%.
The practical significance of this would be determined by each individual buyer in the market for a car since they will have relatively different purchasing power. 
Additionally market conditions that would make economic conditions more or less friendly to for buyers would ultimately affect the digestion or acceptance of this increase in picking models of various years.

- Because cars typically keep a specific model trim for a few years, applying this knowledge to someone shopping for a specific model type can come in handy as they would be able to potentially save money looking for "older" versions of identical models. 
- Alternatively someone with more expendable income and buying power may find that the 1.3% increase is worth buying a "newer" model as a small price to pay for potentially less miles, upgraded technologies, or overall wear on the vehicle. 

*Practical Significance | Log Engine Horse Power Predictor:*
- A doubling of increase of Engine Horsepower is associated with an 80% increase in the Median MSRP. 
Practically speaking when you have a buyer who is looking to maximize vehicle performance (power, speed, acceleration) they are typically looking 
for a specific type of higher performance vehicle which are generally known to cost more compared to vehicles with a more functional purpose. Similar to the Year Predictor
the significance of the Horse Power predictor would depend on the buyer among other factors. 

- Understanding the price to performance (horsepower) trade off when purchasing a vehicle can be of practical significance to buyers looking to predict what a car of a certain performance would cost, as well those looking to find alternative models in which horsepower is not as great. 

*Statistical Significance | Year Predictor:*
- At an alpha = .05 level of significance there is an overwhelming amount of evidence to suggest that 
the Year coefficient is different than zero (p-value < .0001). 


*Statistical Significance | Log Engine Horse Power Predictor:*
- At an alpha = .05 level of significance there is an overwhelming amount of evidence to suggest that 
the Year coefficient is different than zero (p-value < .0001). 
```{r, message=FALSE, warning=FALSE, fig.align='center'}
# removing MSRP 
...
<h3>Testing for Differences in Categorical Variables</h3>
Our easily interpretable model above doesn't include categorical variables due to not understanding if 
the levels within certain variables are different or the same. 

*Hypothesis Testing: Vehicle Size* 
So that we can understand the various categories of the Vehicle Engine Size and if we should account for different slopes between Compact and Large sizes we will perform a hypothesis test which will help us identify significant differences. 
...
Visualizing Slopes For Various Categorical Variables
```{r, message=FALSE, warning=FALSE, fig.align='center'}
backup_df %>% ggplot(aes(log_Engine.HP, log_MSRP, color = Vehicle.Style))+
  geom_point(show.legend = FALSE)+
  geom_smooth(method = 'lm')
backup_df %>% ggplot(aes(log_Engine.HP, log_MSRP, color = Vehicle.Size))+
  geom_point(show.legend = F)+
  geom_smooth(method = 'lm', show.legend = F)
backup_df %>% ggplot(aes(log_Engine.HP, log_MSRP, color = Make))+
  geom_point(show.legend = FALSE)+
  geom_smooth(method = 'lm', show.legend = F)
backup_df %>% ggplot(aes(log_Engine.HP, log_MSRP, color = Engine.Cylinders))+
  geom_point(show.legend = FALSE)+
  geom_smooth(method = 'lm', show.legend = F)
```

<h3>ANOVA Testing</h3>
In the following chunk of code, we run ANOVAs with interactions to assess the effect of a variable on another given it's multiple levels. 
Doing this allows us to determine whether the relationship between the variables is independent or if the interaction modifies the relationship behavior.
```{r, message=FALSE, warning=FALSE, fig.align='center'}
...
<h4>Complex Model</h4>
Using the results of the ANOVAs run above we can determine that various explanatory features have an effect on log_engine.hp. 
So that we can account for these effects we have created interaction terms in our complex model below. 
```{r, message=FALSE, warning=FALSE, fig.align='center'}
...
<hr>
<h4>Feature Selection & 5-Fold Cross Validation</h4>
...
rss <- function(preds, actual){sum((preds - actual) ^ 2)}
# 
# # Total sum of squares (proportional to the variance of the observed data)
tss <- function(actual){sum((actual - mean(actual)) ^ 2)}
# 
# # Coefficient of determination R2
r_square <- function(actual, preds){1 - ((sum((preds - actual) ^ 2))/(sum((actual - mean(actual)) ^ 2)))}
  
r_square(test_y, pred_y_knn)
```
...
```{r}
summary(tree)$adj.r.squared
```


<h3>Conclusion</h3>

Based on the MSE and RMSE, the complex GLMNET model with 5 fold cross validation and LASSO feature selection performed the best with an RMSE of 0.14 and and MSE of 0.02. With only slightly higher metrics, the Regression Tree Model had an RMSE of 0.20 and an MSE of 0.04. The KNN Model had the poorest performance with an RMSE of 0.49 and an MSE of 0.24 

*Measures of Fit:*

|                       |   RMSE    |    MSE     |    MAE    | Adj. R^2  |
|-----------------------|:---------:|:----------:|:---------:|:---------:|
| Complex Model         | 0.1387327 | 0.01854076 |           | 0.7218715 |
| KNN Model             | 0.4900468 | 0.2401459  | 0.3869861 |           |
| Regression Tree Model | 0.1989946 | 0.0395989  | 0.1593347 |           |
