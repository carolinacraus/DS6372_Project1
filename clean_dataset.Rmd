...
Looking for Visual Slope Differences In Categorical Variables
```{r}
# Vehicle Sizes
backup_df %>% ggplot(aes(Year, log_MSRP, color = Vehicle.Size))+
  geom_point()+
  geom_smooth(method = "lm")

# Vehicle Make
backup_df %>% ggplot(aes(Year, log_MSRP, color = Make))+
  geom_point()+
  geom_smooth(method = "lm")

# Engine.Fuel.Type
backup_df %>% ggplot(aes(Year, log_MSRP, color = Engine.Fuel.Type))+
  geom_point()+
  geom_smooth(method = "lm")

# Driven_Wheels
backup_df %>% ggplot(aes(Year, log_MSRP, color = Driven_Wheels))+
  geom_point()+
  geom_smooth(method = "lm")

# Vehivle.Style
backup_df %>% ggplot(aes(Year, log_MSRP, color = Vehicle.Style))+
  geom_point()+
  geom_smooth(method = "lm")

# Engine.Cylinders
backup_df %>% ggplot(aes(Year, log_MSRP, color = Engine.Cylinders)) +
  geom_point()+
  geom_smooth(method = "lm")

# Tranmission.Type
backup_df%>% ggplot(aes(Year, log_MSRP, color = Transmission.Type)) +
  geom_point()+
  geom_smooth(method = "lm")

# Number.of.Doors
backup_df %>% ggplot(aes(Year, log_MSRP, color = as.factor(Number.of.Doors)))+
  geom_point()+
  geom_smooth(method="lm")

# Market.Category
backup_df %>% ggplot(aes(Year, log_MSRP, color = Market.Category))+
  geom_point(show.legend = FALSE)+
  geom_smooth(method = "lm", show.legend = FALSE)

```

Car Size Slope Differnece testing 
```{r}
car_size_slope <- lm(log_MSRP~Year*Vehicle.Size, backup_df)
summary(car_size_slope)
...
contrast1 <- c(0,0,0,0,1,-1) # testing different slopes for large and midsize 

# contrast.matrix<-rbind(contrast1,contrast2,contrast3) if we had multiple contrasts
lm.contrast(car_size_slope,contrast1)
...
car_model_fit <- lm(log_MSRP~Year+log(Engine.HP) + Vehicle.Size,training_slr)
...
```{r}
# linear regression model 
hypo_1 <- lm(log_MSRP~Year*Vehicle.Size + log_Engine.HP,training_slr)
